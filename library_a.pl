...
my($DEBUG)	= "0";
my($inuse)	= `/usr/adic/TSM/bin/fsstate |grep "IN USE"|grep scsi_archive2|wc -l`;
my($free)	= `/usr/adic/TSM/bin/fsstate |grep "FREE"|grep scsi_archive2|wc -l`;
my($delayed)	= `/usr/adic/TSM/bin/fsstate |grep "DELAYED"|grep scsi_archive2|wc -l`;
my($masterFile) = '/usr/adic/install/.active_snfs_server';
my($isMaster)   = "";

# Xymon Variables
$ENV{BBPROG}	= "library_a.pl";
my($TESTNAME)   = "librarya";
